...
Тогда, случайная величина $T$
    \[
        T = \cfrac{z}{\sqrt{\cfrac{\lVert \hu_W \rVert ^2}{\dim(W)}}} =  \tg\alpha\cdot \sqrt{\dim(W)}\sim t_k
    \]
    имеет $t$-распределение с $k$ степенями свободы, где $\alpha$ — угол между $\hu_W \text{ и } \hu_{W+V}$.
\end{geom_def}
...
\begin{geom_def}\hspace{2cm} \par
    ...
    \]
     Пусть $u$ —  вектор пространства $\mathbb{R}^n, u \sim \cN(0, I)$. Тогда $\hu_V$ — проекция $u$ на $V$, $\hu_W$ — проекция вектора $u$ на $W$
    $\hu_{W+V}$ — проекция $u$ на на $W\oplus V$
\begin{center}
\begin{tikzpicture}[x=0.75pt,y=0.75pt,yscale=-1,xscale=1]
...
\begin{proof} \hspace{1cm} \par
    \begin{enumerate}
        ...
        $\hu_L$ — проекция вектора $u$ на $\Colp(X_L),\hspace{0.2cm} \dim(\Colp(X_L))=n-k_L \Rightarrow$
        \par
        $RSS_L=\lVert \hu_L \rVert ^2$ — квадрат длины проекции $u$ на $(n-k_L)$-мерное подпространство
...
% Text Node
\draw (295,163) node   {$\hat{y}^{L} =\ X\hb^{L}$};
% Text Node
\draw (304,51) node [scale=1]  {$y\ =\ X\beta +u$};
% Text Node
\draw (328,193) node   {$\Col X_{L}$};
% Text Node
...
\draw (223,40) node   {$y$};
% Text Node
\draw (303,135) node   {$\Col X^{L}$};
% Text Node
\draw (243,245) node [color={rgb, 255:red, 189; green, 16; blue, 224 }  ,opacity=1 ,rotate=-1.1]  {$\textcolor[rgb]{0.25,0.46,0.02}{\Col X}\textcolor[rgb]{0.25,0.46,0.02}{^{S}}$};
% Text Node
\draw (195,140) node [color={rgb, 255:red, 0; green, 149; blue, 158 }  ,opacity=1 ]  {$\hat{u}^{S}$};
\end{tikzpicture}
        \[
        \textcolor{red}{\hbox{$RSS_S - RSS_L$}}
        = \lVert \hu_S \rVert ^2 - \lVert \hu_L \rVert ^2 
...
\end{center} 
    ...
    \[
        \cfrac{RSS_S-RSS_L}{\s^2}\sim \cChi_{k_L - k_S}^2
    \]
        \item $RSS_L \text{ и } (RSS_S - RSS_L)$ — независимы, поскольку они ортогональны
        \item $\cfrac{(RSS_S - RSS_L)/\left(\s^2\left(k_L - k_S\right)\right)}{RSS_L/\left(\s^2(n-k_L)\right)} = \cfrac{(RSS_S - RSS_L)/(k_L - k_S)}{RSS_L/(n-k_L)} \sim \cF_{k_L-k_S, n-k_L}$
    \end{enumerate}
\end{proof}
