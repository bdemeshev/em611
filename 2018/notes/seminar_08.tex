...
\[ \Sigma E(  {u_i}^2     | X        )  = n \cdot {\sigma}^2\]
\[ \Sigma E(  {{\hat{u}}_i}^2     | X        )  = E(RSS|X) = (n-k) \cdot  {\sigma}^2\]

 \[{\hat{\Sigma}}_{HC_1} = \frac{n} {n-k} {\hat{\Sigma}}_{HC_0}  \]


\textbf{Упр. $HC_2$}
\[ \Sigma E(  {u_i}^2     | X        )  = n \cdot {\sigma}^2\]
\[ \Sigma E(  {{\hat{u}}_i}^2     | X        )  =  \Var( {\hat{u}_i}|X) \]
...
\href
{https://cran.r-project.org/web/packages/sandwich/vignettes/sandwich.pdf} {\textcolor{blue}{здесь.}}
